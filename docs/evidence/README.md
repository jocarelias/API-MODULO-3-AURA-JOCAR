...
| `02-prisma` | `migrate status` __"2 migrations found … up to date"__ · Prisma Studio em `:5555` (index + favicon + log) |
| `03-crud` | HTTP real (curl): health, listas com paginação (total 31/15/50), create assessment/schedule, recálculo lote, impressão, OpenAPI, Swagger UI, delete schedule |
...
| `05-transaction` | Prova ACID: `g3.transaction.e2e.test.js` (2 testes) — rollback forçado no recálculo (nota NÃO persiste) |
| `06-errors` | Envelopes de erro reais: 400 Zod (payload, peso 0, pageSize>100, aluno não matriculado), 404, 409 horário |
| `07-data-quality` | psql real: CHECK constraints (peso>0, nota≤100, horário, average) · UNIQUE indexes (grade por avaliação+aluno; result por aluno+turma+disciplina+período) · contagens 31/101/15/50 · amostra média/status |
| `tests` | Logs completos: `unit-tests.log` (21 passed) e `e2e-tests.log` (25 passed, incl. 2 ACID) |
...
npm run test:e2e           # 25 passed (incl. 2 ACID)
